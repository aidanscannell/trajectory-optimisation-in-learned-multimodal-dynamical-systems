...
class FakeCollocationSVGPQuad:
    ...
    pos_init = np.array([1.5, -2.0])
    pos_init = np.array([-0.5, -2.5])
    pos_init = np.array([1.0, -2.5])
    pos_init = np.array([-1.0, -2.0])
    pos_init = np.array([0.5, -2.0])
    ...
    pos11_guesses = np.linspace(
        pos_init[0], pos_mid[0], int(num_col_points / 2)
    )
    pos21_guesses = np.linspace(
        pos_init[1], pos_mid[1], int(num_col_points / 2)
    )
    pos12_guesses = np.linspace(
        pos_mid[0], pos_end_targ[0], int(num_col_points / 2)
    )
    pos22_guesses = np.linspace(
        pos_mid[1], pos_end_targ[1], int(num_col_points / 2)
    )
...
class FakeCollocationProbSVGPQuad:
    ...
    vel1_guesses = np.linspace(
        vel_init_guess[0], vel_end_guess[0], num_col_points
    )
    vel2_guesses = np.linspace(
        vel_init_guess[1], vel_end_guess[1], num_col_points
    )
    pos1_guesses = np.linspace(pos_init[0], pos_end_targ[0], num_col_points)
    pos2_guesses = np.linspace(pos_init[1], pos_end_targ[1], num_col_points)
    pos_guesses = np.stack([pos1_guesses, pos2_guesses], -1)
...
def test_collocation_prob_svgp_quad():
    ...
    #                                           metric.metric_tensor_fn_kwargs,
    #                                           solver.times)
    geodesic_traj = collocation_root(
        solver.state_guesses,
        solver.pos_init,
        solver.pos_end_targ,
        metric_fn,
