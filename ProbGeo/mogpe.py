...
def inv_probit(x):
    ...
    return 0.5 * (1.0 + jax.lax.erf(x / np.sqrt(2.0))) * (1 -
                                                          2 * jitter) + jitter
    # return 0.5 * (1.0 + np.scipy..erf(x / np.sqrt(2.0))) * (1 -
    #                                                      2 * jitter) + jitter
...
def mogpe_mixing_prob_jacobian(Xnew: InputData,
                               X: InputData,
                               kernel,
                               mean_func: MeanFunc,
                               f: OutputData,
    ...
    def single_jac(x_new):
        ...
        jac = jacfwd(mogpe_mixing_probability,
                     (0))(x_new, X, kernel, mean_func, f, full_cov, q_sqrt,
                          jitter, white)
        return jac
