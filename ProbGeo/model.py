...
class BMNSVGP(Model):
    ...
    def _sample_e_h(self, dist_h, f_means, num_samples=1000):
        ...
        p_a_0 = self.likelihood.predict_prob_a_0(h)
    ...
    def _sample_e_f(self, dist_fs, h_mean, h_var, num_samples=1000):
        ...
        p_a_0 = self.likelihood.predict_prob_a_0(h_mean)
    ...
    def _var_expectation(self,
        ...
        # return 1. / (num_samples_f * num_samples_h) * tf.reduce_sum(
        #     sum, axis=[0, 1])  # [num_data, ]
        return 1. / (num_samples_f * num_samples_h) * tf.reduce_sum(
            sum)  # [num_data, ]

    @params_as_tensors
    def _var_expectation_analytic(self, f_means, f_vars, p_a_0):
        p_y = []
        for f_mean, f_var, noise_var in zip(f_means, f_vars,
                                            self.likelihood.variances):
            print('var_exp_analytic')
            print(f_var)
            print(noise_var)
            variance = f_var + noise_var

            dist_y = tfp.distributions.MultivariateNormalFullCovariance(
                loc=f_mean, covariance_matrix=variance)
            py = dist_y.prob(self.Y)  # [num_data, ]
            p_y.append(py)
        sum = tf.log(p_y[0] * p_a_0 + p_y[1] * (1 - p_a_0))
    ...
    def _build_likelihood(self):
        ...
        # Analytically calculate p(a=0 | x)
        # h = h_mean / (tf.sqrt(1 + h_var))
        # p_a_0 = self.likelihood.predict_prob_a_0(h)
        # p_a_0 = 1 - inv_probit(h_mean / (tf.sqrt(1 + h_var)))

        p_a_0 = self.likelihood.predict_mean_and_var_a(h_mean, h_var)[0]
        p_a_0 = tf.reshape(p_a_0, [-1])  # [num_data, ]

        # Gibbs sample to calculate p(a=0 | x)
        num_samples_h = 1
        # h = dist_h.sample(num_samples_h)  # [num_samples x num_data x 1]
        # p_a_0 = self.likelihood.predict_prob_a_0(h)
        # p_a_0 = tf.reshape(p_a_0,
        #                    [num_samples_h, -1])  # [num_samples x num_data]

        # Sample from function GPs
        num_samples_f = 1
        f_vals = []
        for dist_f in dist_fs:
            f = dist_f.sample(
                num_samples_f)  # [num_samples x num_data x output_dim]
            f_vals.append(f)

        # Calculate variational expectation for each data point
        # var_exp = self._var_expectation(f_vals, p_a_0, num_samples_f)
        f_means_expanded = []
        f_vars_expanded = []
        for f_mean, f_var in f_means, f_vars:
            f_mean = tf.expand_dims(f_mean, 0)
            f_var = tf.expand_dims(f_var, 0)
            f_means_expanded.append(f_mean)
            f_vars_expanded.append(f_var)
        var_exp = self._var_expectation_analytic(f_means_expanded,
                                                 f_vars_expanded, p_a_0)

        # var_exp = self._var_expectation(f_means, p_a_0, num_samples_f,
        #                                 num_samples_h)
        # var_exp = self._var_expectation(f_vals, p_a_0, num_samples_f,
        #                                 num_samples_h)
    ...
    def predict_y(self, Xnew):
        ...
        # p_a_0 = self.likelihood.predict_prob_a_0(h)
        p_a_0 = self.likelihood.predict_mean_and_var_a(h_mean, h_var)[0]
        # y_mean = y_means[0] * (1 - p_a_0) + y_means[1] * p_a_0
        # y_var = y_vars[0] * (1 - p_a_0) + y_vars[1] * p_a_0 + y_means[0]**2 * (
        #     1 - p_a_0) + y_means[1]**2 * (p_a_0) - ((y_means[0] *
        #                                              (1 - p_a_0) + y_means[1] *
        #                                              (p_a_0))**2)

        y_mean = y_means[1] * (1 - p_a_0) + y_means[0] * p_a_0
        y_var = y_vars[1] * (1 - p_a_0) + y_vars[0] * p_a_0 + y_means[1]**2 * (
            1 - p_a_0) + y_means[0]**2 * (p_a_0) - ((y_means[1] *
                                                     (1 - p_a_0) + y_means[0] *
                                                     (p_a_0))**2)
        # y_var = y_vars[1] * (1 - p_a_0) + y_vars[0] * p_a_0
        # TODO Account for cross covariance in y_var
        return y_mean, y_var
