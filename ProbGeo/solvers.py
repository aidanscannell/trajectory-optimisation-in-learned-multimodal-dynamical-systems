...
def integrate_multiple_guesses(state_guesses, times, ode_args):
    ...
    for i in range(num_grid_points - 1):
        state_integrated = integrate_guesses_scipy(ode_fn_scipy,
                                                   state_guesses[i, :],
                                                   times[i, :])
        states_integrated = jax.ops.index_update(states_integrated,
                                                 jax.ops.index[i + 1, :],
                                                 state_integrated)
...
def shooting_error(state_guesses, pos_start, pos_end, metric_fn,
    ...
    state_guesses = jax.ops.index_update(state_guesses,
                                         jax.ops.index[0, :input_dim],
                                         pos_start)
    state_guesses = jax.ops.index_update(state_guesses,
                                         jax.ops.index[-1, :input_dim],
                                         pos_end)
    # print('state guesses after update')
    # print(state_guesses)
    ode_args = [metric_fn, metric_fn_kwargs]
    states_integrated = integrate_multiple_guesses(state_guesses, times,
                                                   ode_args)
    ...
    # print('states in')
    # print(states_integrated.shape)
    # print(state_guesses.shape)
    # error = np.subtract(pos_integrated, pos_guesses)
    # error = np.subtract(states_integrated, state_guesses[1:, :])
...
def multiple_shooting(state_guesses, metric_fn, metric_fn_kwargs, times):
    ...
    shooting_error_args = (pos_start, pos_end, metric_fn, metric_fn_kwargs,
                           times)
    ...
    print(state_guesses)
    print(state_guesses.shape)
    opt_result = root(shooting_error,
                      state_guesses,
    ...
    state_opt = jax.ops.index_update(state_opt, jax.ops.index[-1, :input_dim],
                                     pos_end)
    print(state_opt.shape)
    print(state_opt)
