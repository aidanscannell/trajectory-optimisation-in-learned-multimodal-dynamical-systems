...
# def calc_vecG_global(c, X, Y, kernel):
#     # G = calc_G(c, X, Y, kernel)
#     global G
#     G, _, _ = calc_G_map_global(c, X, Y, kernel)
#     input_dim = X.shape[1]
...
def geodesic_fun(c, g, X, Y, kernel):
    ...
    # grad_vecG = jacfwd(calc_vecG(c, X, Y, kernel), 0)
    # print(grad_vecG)
    dvecGdc = grad_vecG(c, X, Y, kernel)
    # print(c.shape)
    # vgrad_vecG = vmap(value_and_grad(calc_vecG_map, 0), (1, None, None, None))
    # dvecGdc = vgrad_vecG(c, X, Y, kernel)
