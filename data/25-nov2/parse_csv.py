...
# df = data_tello
#     # events = np.split(df, np.where(np.isnan(df.index))[0])
#     events = np.split(df.to_numpy(), np.where(np.isnan(df.index))[0], axis=1)
#     print(len(events))
#     # removing NaN entries
#     events = [
#         ev[~np.isnan(ev.value)] for ev in events
#         if not isinstance(ev, np.ndarray)
#     ]
#     # removing empty DataFrames
#     events = [ev for ev in events if not ev.empty]

#     print("Found " + str(len(events)) + " trials using empty lines")
...
#     idxs = np.where(np.isnan(df.index))[0]
...
def parse_traj_csv_by_empyt_line(file_name):
    data_tello = pd.read_csv(file_name, index_col=0)
...
def parse_single_vicon_csv(file_name, freq=100):
    ...
    print("vicon start index: ", data_vicon.index[0])
