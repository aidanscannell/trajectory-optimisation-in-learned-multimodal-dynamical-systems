...
filename = "./saved_models/16-4/141/model"
...
fig, ax = plot_contourf(
    ...
    cbar_label='$P(\\alpha_*=0 | \mathbf{x}_*, \mathcal{D}, \mathbf{\\theta})$',
    levels=levels)
plt.savefig(save_path + 'alpha_discrete_4_6.pdf', transparent=True)
levels = [0, 0.3, 0.7, 1]
fig, ax = plot_contourf(
    xx,
    yy,
    ...
    cbar_label='$P(\\alpha_*=0 | \mathbf{x}_*, \mathcal{D}, \mathbf{\\theta})$',
    levels=levels)
plt.savefig(save_path + 'alpha_discrete_3_7.pdf', transparent=True)
...
for i, (f_mu, f_var) in enumerate(zip(f_mus, f_vars)):
    ...
    plot_mean_and_var(xx,
                      ...
                      fig=fig,
                      axs=axs[i, :])
plt.savefig(save_path + 'f_combined.pdf', transparent=True)
